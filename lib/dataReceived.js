...
module.exports = function (data, resolve) {
	if ((data != null) && (data != "")) {
		...
		var timestamp = +datas[0];
		var nodeId = +datas[1];
		var sensorId = +datas[2];
		var messageType = +datas[3];
		var ack = +datas[4];
		var subType = +datas[5];
		var rawpayload = "";
		if (datas[6]) {
			rawpayload = datas[6].trim();
		}
		var payload;
		...
		switch (messageType) {
			...
			case constants.C_SET:
				saveInformations.saveValue(nodeId, sensorId, subType, payload, timestamp)
					.then(function() {
						resolve();
					});
				break;
